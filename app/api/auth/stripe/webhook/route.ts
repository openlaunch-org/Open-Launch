...
import { project, launchQuota } from "@/drizzle/db/schema";
import { eq, sql } from "drizzle-orm";
import { launchStatus, launchType } from "@/drizzle/db/schema";
      ...
      if (session.payment_status === "paid") {
        ...
        if (quotaResult.length === 0) {
          ...
          await db.insert(launchQuota).values({
            ...
            premiumPlusCount:
              projectData.launchType === launchType.PREMIUM_PLUS ? 1 : 0,
            createdAt: new Date(),
            updatedAt: new Date(),
          });
        } else {
