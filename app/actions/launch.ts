...
export async function getLaunchAvailability(
  ...
  const premiumSlots = Math.max(
    0,
    LAUNCH_LIMITS.PREMIUM_DAILY_LIMIT - premiumCount
  );
  const premiumPlusSlots = Math.max(
    0,
    LAUNCH_LIMITS.PREMIUM_PLUS_DAILY_LIMIT - premiumPlusCount
  );
...
export async function getLaunchAvailabilityRange(
  ...
  const adjustedStartDate = isBefore(parsedStartDate, minDate)
    ? minDate
    : parsedStartDate;

  // Ajuster la date de fin si elle est après la date maximale
  const adjustedEndDate = isBefore(maxDate, parsedEndDate)
    ? maxDate
    : parsedEndDate;
...
export async function scheduleLaunch(
  ...
  try {
    ...
    if (normalizedParsedDate < minDate) {
      throw new Error(
        `La date de lancement doit être au moins ${minDaysAhead} jour(s) à l'avance`
      );
    }
    ...
    if (
      ...
      launchTypeValue === LAUNCH_TYPES.PREMIUM_PLUS
    ) {
